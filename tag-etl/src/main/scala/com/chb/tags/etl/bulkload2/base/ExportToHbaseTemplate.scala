package com.chb.tags.etl.bulkload2.base

import java.nio.ByteBuffer
import java.util
import java.util.Properties

import com.chb.tags.etl.bulkload2.util.ReadProperties
...
abstract class ExportToHbaseTemplate(outPutTable: String, APPName: String) {
  ...
  val proInstance: Properties = ReadProperties.getInstance(proPath).getProInstance
...
//  }
   ...
   * HFile 导入HBase
   */
  def loadHFileToHbase(): util.Map[LoadIncrementalHFiles.LoadQueueItem, ByteBuffer] = {
   ...
   * @param hfileRDD
   */
  def saveHfile(hfileRDD: RDD[(ImmutableBytesWritable, KeyValue)]): Unit = {
